...
def indexer(path):
    ...
    tfidf_max = 0
    tfidf_min = 10000
    DocumentNum=0 # Record the number of documents processed.
    print("Indexing!")
    # C:\Users\USER\Downloads\WEBPAGES_RAW
    #filePathBase = "C:/Users/USER/Downloads/WEBPAGES_RAW"
    filePathBase=path+"/WEBPAGES_RAW"
    Index = {}
    global res
    res=[]
    #MapFile = "C:/Users/USER/Downloads/WEBPAGES_RAW/bookkeeping.json"
    MapFile = path+"/WEBPAGES_RAW/bookkeeping.json"
    ...
    for i in range(75):
        ...
        for j in range(N):
            ...
            with open(filePath) as f:
                ...
                if soup.head is None and soup.body is None: 
                    print "Strange file: %s" % document
                    continue

                print "process file: %s" % filePath
                DocumentNum += 1
...
def check_dir(initial):
    filename='C:/Github/CS-221-project3-search-engine/index/'+initial[0]+'/'+initial[1]+'/'+initial[2]+'.json'
    ...
    for term in indexdict:
        ...
        if len(term[0])>2:
            if (term[0][0]+term[0][1]+term[0][2])!=initial:
                if len(cur)>0:
                    with open('C:/Github/CS-221-project3-search-engine/index/'+initial[0]+'/'+initial[1]+'/'+initial[2]+'.json', 'w') as outfile:
